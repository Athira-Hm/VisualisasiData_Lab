...
d3.csv("Mental_Health_Lifestyle_Dataset.csv").then(function(rawData) {
  ...
  const data = Array.from(nested, ([Country, levels]) => ({ Country, ...levels }));
  ...
  const x = d3.scaleBand()
    .domain(groups)
    .range([0, width])
    .padding([0.05]); // 📌 padding dikurangi supaya lebih rapat
  ...
  svg.append("g")
    ...
    .join("rect")
      ...
      .on("mouseover", function(event, d) {
        ...
        d3.select(this).attr("opacity", 0.7); // efek hover pada bar
      })
